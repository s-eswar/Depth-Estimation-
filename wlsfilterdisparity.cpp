...
const String keys =
    ...
    "{vis_mult       |3.0               | coefficient used to scale disparity map visualizations            }"
    "{max_disparity  |112               | parameter of stereo matching                                      }"
    "{window_size    |11                | parameter of stereo matching                                      }"
...
int main(int argc, char** argv)
{
    ...
    if(!no_display)
    {
        ...
        Mat sfalseColorsMap;
        applyColorMap(raw_disp_vis, sfalseColorsMap, cv::COLORMAP_JET);
        imshow("raw disparity", raw_disp_vis);
        imwrite("raw_disparity_color.jpg", sfalseColorsMap);
        Mat filtered_disp_vis;
        getDisparityVis(filtered_disp,filtered_disp_vis,vis_mult);
        namedWindow("filtered disparity", WINDOW_AUTOSIZE);
        imshow("filtered disparity", filtered_disp_vis);
        Mat falseColorsMap;
        applyColorMap(filtered_disp_vis, falseColorsMap, cv::COLORMAP_JET);
        imwrite("filtered disparity.jpg", filtered_disp_vis);
        imwrite("filtered disparity_color.jpg", falseColorsMap);
